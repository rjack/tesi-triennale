...
mittente e incapsulata in pacchetti UDP che vengono inviati al
destinatario via Internet. Poiché i dati percorrono reti progettate
per un utilizzo generico, utilizzando il protocollo UDP che non
fornisce alcun tipo di garanzia, può accadere che in determinate
circostanze la rete non sia in grado di fornire un servizio adatto a
una comunicazione VoIP.
...
banda larga si sono affiancate le reti senza fili (WiFI, 802.11,
WiMax), che stanno diventando una realtà sempre più diffusa grazie
alla loro semplicità di installazione, praticità di utilizzo ed
economicità dei componenti. Lo scenario tipico consiste in una rete
WiFi che, attraverso uno o più \emph{access point}, offre accesso a
una rete broad band cablata per svariati dispositivi mobili; in questi
casi il traffico di rete deve attraversare almeno un segmento do rete
wireless prima di giungere alla rete via cavo.
...
minato da interferenze, collisioni tra pacchetti e indebolimento del
segnale radio con l'aumentare della distanza tra dispositivo e access
point. Il requisito di interattività viene violato ogniqualvolta vi
sia la necessità di ritrasmettere i pacchetti che sono stati persi
durante un'errore di trasmissione. Inoltre, la mobilità dei
dispositivi senza fili introduce il problema inedito di quella che si
potrebbe chiamare migrazione trasparente di connettività. Attualmente
un dispositivo connesso ad una rete senza fili è limitato nella sua
mobilità alla copertura di rete dell'access point utilizzato: se,
durante uno spostamento, il dispositivo esce dalla copertura di una
rete WiFi ed entra nella copertura di una seconda rete WiFi, la
procedura di disconnessione dalla prima rete e riconnessione alla
seconda è distruttiva e interrompe ogni tipo di connessione in corso.
...
di punti di accesso a diverse reti wireless, pubbliche e private. In
questo scenario sono necessari meccanismi e protocolli che permettano
l'uso di applicazioni VoIP in condizioni di estrema mobilità,
garantendo le proprietà di interattività e affidabilità necessarie.

In questo documento (TODO: tesi, lavoro, opera?) presenterò diversi
meccanismi sperimentali che puntano a risolvere questo problema,
mettendone in luce pregi e difetti e confrontando le varie soluzioni proposte.
