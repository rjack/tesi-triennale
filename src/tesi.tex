...
Mobile IP v6 (MIP) -- Network layer
...
Host Identity Protocol (HIP) -- Tra Network e Transport layer
...
Datagram Congestion Control Protocol (DCCP) e Mobile Stream Control
Transport Protocolol (m-SCTP) -- Transport layer
...
Terminal Mobility Support Protocol (TMSP) -- Session layer
...
### SOLUZIONI CROSS LAYER ###

MONA (riassunto paper)
* si concentra sul problema dell'handover, approccio multi-homed
* usa un protocollo cross-layer che usa il numero di ritrasmissioni L2
  per decidere quando è il momento di fare handoff
* media selection basata sulla condizione del first-hop
* no modifica ad applicazioni esistenti
* selezione del media flow per flow, a seconda del tipo di flow
* cerca di bilanciare il carico di lavoro degli AP con una modifica
  all'802.11
* association raggruppa flow tra due host e viene mantenuta con
  signalling: no server esterni.
