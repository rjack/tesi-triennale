...
La prima analisi mira a individuare i valori migliori per le soglie di
passaggio da Single Path a Multi Path $v_{MPT}$ e da Multi Path a
Single Path $v_{SPT}$. Come da tabella \ref{tab:mona:vincoli} si
devono selezionare i valori in cui il packet loss misurato è minore
del 3\%.
...
\begin{figure}
  ...
  \begin{tabular}{cc}
    ...
    c) & \includegraphics[width=9cm]{img/mona-sim-packet-loss-rate}  \\
  \end{tabular}
  \caption{\em Misurazioni durante l'handover di a) percentuale di
    ...
    c) distribuzione di pacchetti persi consecutivi.}
  \label{fig:mona:qos-sim-model}
\end{figure}
