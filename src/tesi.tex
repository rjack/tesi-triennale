...
Alta mobilità uguale a handover frequenti. Normalmente handover dura
dai 50 ai 400 ms. L'acquisizione di un indirizzo di rete via DHCP può
durare secondi; uso di più interfacce di rete (multi--homing) per
evitare interruzioni nel passaggio tra una rete e l'altra.

\paragraph{Identità.}
Association raggruppa flow tra due host e viene mantenuta con
signalling: no server esterni.

\paragraph{Traffico real time non real time.}

Selezione del media flow per flow, a seconda del tipo di flow. Cerca
di bilanciare il carico di lavoro degli AP con una modifica
all'802.11.

\paragraph{QoS per collegamento.}

Numero di ritrasmissioni L2 è una metrica migliore della qualità del segnale.

\paragraph{Compatibilità.}
No modifica ad applicazioni esistenti. Se CN non implementa MONA,
fallback a IP normale. Se AP non implementa modifica a 802.11, TODO:
che succede?
