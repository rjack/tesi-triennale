...
Molti servizi interattivi multimediali su Internet fanno uso del
Session Initiation Protocol (SIP) per stabilire una sessione di
comunicazione.

Questa sezione presenta una breve panoramica del protocollo SIP,
mostrando i ruoli degli host, la gestione della sessione e alcuni
limiti dovuti alla presenza di firewall e NAT.
...
In uno scenario VoIP, l'applicazione utilizza il protocollo SIP per
inviare al server SIP un messaggio REGISTER, che segnala che l'utente
è collegato e pronto a ricevere e inviare chiamate. La chiamata viene
effettuata mediante un messaggio INVITE, che specifica
l'identificativo dell'utente da contattare e i dati dell'utente
chiamante; il server recupera l'indirizzo IP dell'utente richiesto, e
...
conversazione, l'applicazione invia un messaggio REINVITE con i nuovi
parametri, per permettere al server di reinstaurare la sessione con le
caratteristiche aggiornate.

TODO: controllare meccanismo REINVITE. Il meccanismo REINVITE è
intrinsecamente lento: il mittente deve contattare il server, il
server deve a sua volta contattare il destinatario e solo a quel punto
la comunicazione tra mittente e destinatario può continuare nella
nuova sessione. Nonostante questo procedimento possa avvenire in
automatico, ovvero senza intervento manuale dell'utente, la
comunicazione vocale deve essere interrotta finché non viene
instaurata la nuova sessione e questo rovina l'esperienza utente.

ABPS definisce delle scorciatoie di segnalazione per rendere il
procedimento di REINVITE diretto tra i due partecipanti, quindi senza
che sia necessario interpellare il server SIP.

\subsection{Problematiche dovute a Firewall e NAT}

Il protocollo SIP si occupa soltanto di instaurare il contesto della
comunicazione, permettendo ai partecipanti di specificare le
caratteristiche della sessione, instaurata la quale lo scambio di dati
avviene in maniera diretta tra i due host. Alla luce di ciò, NAT e
firewall sono impedimenti perché non permettono a un host di essere
direttamente raggiungibile dalla controparte. Firewall e NAT sono
spesso di proprietà dell'organizzazione che offre connettività,
soprattutto in un contesto WAN, per cui l'utente non ha possibilità di
impostare soluzioni di port-forwarding. Il metodo più comune prevede
che l'utente imposti l'applicazione con l'indirizzo di un server STUN
o TURN in modo da utilizzare uno di questi servizi. Purtroppo STUN
...
ABPS modifica il server SIP in modo che sia anche relay opaco, ovvero
che nasconda l'indirizzo IP del sistema utente. In questo modo il
server ABPS-SIP si occupa di rappresentare pubblicamente il MN, che
può essere così raggiungibile da altri nodi.
